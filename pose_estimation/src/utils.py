...
def plot_3d(q_start3d, q_joints, joint_parents,savepath,savegif):
    q_start3d.get()
    ...
    ani_update = PoseAnimation3d(ax, joint_parents)
    global ani
    ani = FuncAnimation(fig, ani_update, frames=joints_iter_gen_inner, init_func=ani_update.ani_init, interval=100,
                        blit=True)
    if savegif:
        ani.save(savepath+".gif", dpi=80, writer='imagemagick')
    plt.show()
